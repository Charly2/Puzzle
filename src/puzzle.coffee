class PuzzleCellView
  ...
  constructor: ({@number, @controller}) ->
    ...
    @node.html("<img src='img/"+@number+".jpg'/>")
  ...
  handleSolveClicked: ->
    if @view.isInteractive() and not @grid.isSolved()
      @view.hideControls =>
        ...
        @view.showOverlay 'Resolviendo', =>
          solve @grid, {
            complete: ({steps}) =>
              @view.hideOverlay =>
                @applyMoves steps, =>
                  @view.showControls()
                  swal("Felicidades", "Se resolvio corectamente en "+ steps.length+" pasos","success");
